...
static float warning_level = 1700;
namespace esphome
{
  namespace homekit
  {
        ...
        if (acc) {
          ...
          if (hs) {
              ...
              if (std::equal(device_class.begin(), device_class.end(), strdup("carbon_dioxide"))) {
                  ...
                  ESP_LOGD(TAG, "get is co2 detect char_t - f:%.2f u:%lu", hap_char_get_val(on_char)->f, hap_char_get_val(on_char)->u);
                  hap_char_update_val(on_char, &det_val);
          ...
          ESP_LOGD(TAG, "Read called for Accessory %s (%s)", std::to_string(sensorPtr->get_object_id_hash()).c_str(), sensorPtr->get_name().c_str());
            ...
            if (std::equal(device_class.begin(), device_class.end(), strdup("carbon_dioxide"))) {
                ...
                if (hc == char_co2_level) {
                    ESP_LOGI(TAG,"reader setting detected co2 level");
                    // 从你的传感器读取实时数据
                    val.f = v;  // float
...
//            hap_char_t *char_co2_detected = nullptr;
            ...
            // 添加 CarbonDioxideDetected 疑似默认会添加一个
//            // 0 = 正常, 1 = 检测到高浓度
